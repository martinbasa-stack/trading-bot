...
from src.strategy.trades.models import AverageSum
...
# AssetAnalyzer Class (short-living)
# ----------------------------------------------------------------------
class AssetAnalyzer:
    ...
    def __init__(self, 
        ...
        """
        #functions
        self._get_strategy_by_id :callable = get_by_id
        self._get_all_avg :callable = get_all_avg
        self._get_available_balance :callable = get_available_balance
        self._get_close :callable = get_close
        ...
        self._s1_account_balance: float = 0.0
        self._s2_account_balance :float = 0.0
    ...
    def get_compute(self, strategy_id, buy_factor, sell_factor) -> AssetManagerResult:
        ...
        try:
            with self._lock:
                ...
                self._to_buy  = min(self._to_buy , self._s2_available_balance, self._s2_account_balance) 
                self._to_sell = min(self._to_sell, self._s1_available_balance * self._last_close, self._s1_account_balance* self._last_close)
    ...
    def get_available(self, symbol: str) -> float:
        ...
        try:            
            with self._lock:
                return self._get_available_balance(symbol)
    ...
    def _populate_instance_vars(self, strategy_id):        
        ...
        self._config = copy.copy(strategy.asset_manager)
        ...
        if not self._config.paper_t:
            self._s1_available_balance = self._get_available_balance(strategy.symbol1)# *0.9999
            self._s2_available_balance = self._get_available_balance(strategy.symbol2)# *0.9999

        self._s1_account_balance = self._s1_available_balance 
        self._s2_account_balance = self._s2_available_balance
