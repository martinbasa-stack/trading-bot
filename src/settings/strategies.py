...
class StrategyManager:
    ...
    def __init__(self, 
        ...
        self._old_strategies: List[StrategyConfig] = self._load_json()
    ...
    def _save_json(self):
        ...
        try:
            old = [strategy_to_dict(x) for x in self._old_strategies]
            json_ready = [strategy_to_dict(x) for x in self._strategies]
            save_json(self._file_path, json_ready)
            self._log_changes(old, json_ready)
            self._old_strategies = copy.deepcopy(self._strategies)
        except Exception as e:
    ...
    #vReturn list of pairs wit symbols s1 and s2
    # ------------------------------------------------------------------
    def get_pairs(self, filter_only = None, filter_exclude = None) -> dict[str,dict]:
        """ 
        Args:
            filter_only(str, Optional):
                If provided only pairs from strategy with provided string in type will be returned
        Returns:
            dict[str,dict]:
                Dict of strategy pairs:
                    'BTCUSDC'{
                        "s1" : 'BTC'
                        "s2" : 'USDC'
                    },...
        ...
        with self._lock:        
            ...
            for s in self._strategies:
                if filter_only:
                    if filter_only not in s.type_s:
                        continue
                
                if filter_exclude:
                    if filter_exclude in s.type_s:
                        continue
    ...
    def set_run(self, strategy_id, value: bool) -> None:
        ...
        with self._lock: 
            ...
            if idx is None:
                return None
            self._strategies[idx].run = value
    ...
    def set_paper_t(self, strategy_id: int, value: bool) -> None: 
        ...
        with self._lock: 
            ...
            if idx is None:
                return None
            self._strategies[idx].asset_manager.paper_t = value
    ...
    # ------------------------------------------------------------------
    def generate_pairs_intervals(self, filter_t = "all") ->  dict[str, dict[str, object]]:
        """   
        Args:
            filter_t(str, Optional):
                Filters only provided strategy type. 'all' will provide all.                    
        Returns:
        ...
        with self._lock:
            ...
            for s in self._strategies: #run trough all the stratagies
                if filter_t not in s.type_s and filter_t != "all":
                    continue
    ...
    # Change Logger
    # ==================================================================
    def _log_changes(self, old_list :list[dict], new_list:list[dict]):
    ...
    def _get_index_by_id(self, strategy_id) -> int:
