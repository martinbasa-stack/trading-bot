        ...
        if request.method == 'POST' and request.form:
            ...
            match action:
                case "ADD":
                    ...
                    en_savings = bool(request.form.get("en_savings"))   
                    min_deposit = float(request.form.get("min_deposit"))   
                    min_apy = float(request.form.get("min_apy"))        
                    new_t = solana_man_obj.tokens.new_token(mint, en_savings, min_deposit, min_apy)
                    if not new_t:
