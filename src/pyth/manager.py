...
class PythDataManager:     
    ...
    def __init__(self,
        ...
        # Data monitoring        
        self._count_no_data: int = 0
        self.max_no_data : int = 4
        self.time_old_data : int = 40 #in seconds
    ...
    def _monitor_data_integrity(self):
        time_old_data = self.time_old_data
        ...
        if (
            ...
            self._count_no_data += 1
            self._data_error_time_reduction = time_oldest
            if self._count_no_data > 0:
                timestamp_oldest = self.oldest_timestamp()
                self._logger.warning(
                    f"StreamWorker no data received: {self._count_no_data} | time oldest= {time_oldest} s | timestamp of oldest stream= {datetime.fromtimestamp(int(timestamp_oldest))} s"
                )                
                print(f"Pyth Stream no data count: {self._count_no_data} | time oldest= {time_oldest} s | timestamp of oldest stream= {datetime.fromtimestamp(int(timestamp_oldest))} s")
                #self._stream.ping()
                # Unsubscribe
                #self._requested_pairs.clear()
                #self._unsubscribe()

            if self._count_no_data > self.max_no_data:
