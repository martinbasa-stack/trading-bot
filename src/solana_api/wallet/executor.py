...
from solders.solders import TransactionStatus, Signature, GetTransactionResp, TransactionConfirmationStatus
...
class SolanaWalletExecutor:
    ...
    def sign_send(self,tx_id:str, tx_base64:str, rpc:str, auto_delete: bool = False) -> str:
        """
        ...
        try:
            ...
            with self._lock:
                print("debug executor Sign and send ---------------------")
                print(f"ID: {tx_id}")
                ...
                tx = self.generate_versioned_tx(tx_base64)
                # Sign
                ...
                self._logger.debug(f"ID: {tx_id} PendingTrans:{self._pending[tx_id]}")
                return self._pending[tx_id].signature
    ...
    def _load_confirmation(self, signature_str: str) -> TransactionStatus:
        ...
        resp = client.get_signature_statuses([s],search_transaction_history=False)

        if resp is None:
            resp = client.get_signature_statuses([s],search_transaction_history=True)
        
        if resp is None:
            raise RuntimeError(f"check_confirmation: No response recived for signature: '{signature_str}'!")
        ...
        if stat is None:
            raise RuntimeError(f"check_confirmation: No status for signature: '{signature_str}' response: '{resp.value}'!")
    ...
    @staticmethod
    def generate_versioned_tx(tx_base64:str) -> VersionedTransaction:
