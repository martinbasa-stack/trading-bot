...
class KaminoLending:
    """
    Kamino Lending interaction getting pool data and generating transaction for deposit and withdraw.
    """
    ...
    def get_all_deposits(self, wallet_pub: str) -> dict[str, UserBalances]:
        ...
        try:
            ...
            with self._lock:    
                user_reserves = None        
                user_reserves = self._user_reserve_deposits(wallet_pub)
                for d in user_reserves:
                    if int(d.get("depositedAmount")) > 0:
                        reserve = d.get("depositReserve")
            ...
            self._logger.error(f"get all deposits error: {repr(e)} - user reserves: {user_reserves}")
    ...
    def _full_reserve_metrics(self, reserve:str) -> dict:
        ...
        try:
            ...
            td = datetime.fromtimestamp(now - (2*3600),timezone.utc)
