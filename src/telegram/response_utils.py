...
def strategy_status(idx:str) -> str:
    ...
    asset_resault = asset_analyzer_obj.get_compute(idx, compute_dca.buy_factor, compute_dca.sell_factor)
    ...
    msg = (
        ...
        f"-------------------------------\n"
        f"Balance: \n"
        f"  To sell {_custom_round(asset_resault.to_sell / last_close)} {s.symbol1} \n"
        f"  Available {_custom_round(avail_s1)} {s.symbol1} \n"
        f"  To buy {_custom_round(asset_resault.to_buy)} {s.symbol2} \n"
