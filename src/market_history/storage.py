...
def delete_csv(paths_to_keep: list[Path]):
    """
    ...
    for file in data_dir.iterdir():
        if file.is_file() and "_candle_" in file.name:
            if file.resolve() not in keep_set:
                print(f"DELETE: {file}")
                file.unlink()

def build_candle_path(symbol1 :str, symbol2:str, interval:str):
    """
    ...
    return (
        BASE_DIR
        / "data"
        / f"_{symbol1}_{symbol2}_candle_{interval}.csv"
    )
