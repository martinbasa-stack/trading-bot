...
class StreamWorker:
    ...
    def __init__(
        ...
        generate_pairs_intervals : callable,
        ...
        Args:
            ...
            generate_pairs_intervals(callable): 
                Function to retrive pairs with their intervals from Strategymanager.
    ...
    async def _connection_loop(self):
        ...
        try:
            ...
            with self._comm_lock:
                self._stream_manager._connected = False
        ...
        for pair in requested_streams:
            if pair not in self._active_streams:
                try:
                    ...
                    print(f"Subscribed to stream: {pair} | interval={self._interval}")
        ...
        for pair in list(self._active_streams.keys()):
            if pair not in requested_streams:
                try:
                    ...
                    print(f"Unsubscribed from stream: {pair}")
                except Exception as e:
        ...
        if (
            ...
            if self.count_no_data > 0:
                ...
                self._logger.warning(
                    f"StreamWorker no data received: {self.count_no_data} | time oldest= {time_oldest} s | timesramp of oldest stream= {datetime.fromtimestamp(int(timestamp_oldest))} s"
                )
    ...
    async def _cleanup(self, connection: SpotWebSocketStreams):
        ...
        print("StreamWorker cleanup complete")
    ...
    def _global_cleanup(self):
        try:
            for stream in list(self._strem_map.keys()):
                ...
                print(f"StreamWorker global cleanup stream: {stream}")
        except Exception as e:
