...
from src.strategy.assets import Balance
from src.strategy.trades import Trade
...
class WebsocketManager:
    ...
    def get_exchange_info(self) -> dict:
        ...
        """
        with self._internal_lock:
            return copy.deepcopy(self._exchange_info_data)
    ...
    def ping_ws(self) -> PingResponse:
        ...
        try:            
            ...
            with self._comm_lock: #wait for result   
                if rand_uuid != self._ws_cmds.id:
                    self._logger.error(f"ping_ws() error: uuid mismatch send= {rand_uuid} response= {self._ws_cmds.id}")  
                self._ws_cmds.id = ""   

                self._logger.debug(f"ping_ws() return: {self._ws_cmds.respons_data}")
                formated_resp = self._format_ping_response(self._ws_cmds.respons_data)
                
            with self._internal_lock:
                self._last_ping_response = formated_resp
                return self._last_ping_response
        except Exception as e:
            self._logger.error(f"ping_ws() error: {e}")
            return f"ping_ws() error: {e}"
    ...
    def fetch_user_data(self) -> Dict[str, Balance]:
        ...
        try:
            ...
            with self._comm_lock: #wait for result                
                if rand_uuid != self._ws_cmds.id:
                    self._logger.error(f"fetch_user_data() error: uuid mismatch send= {rand_uuid} response= {self._ws_cmds.id}")  
                self._ws_cmds.id = ""   
            ...
            self._logger.error(f"fetch_user_data() error: {e} \n response -> {self._ws_cmds.respons_data}")
            return None
    ...
    def fetch_exchange_info(self, force=False) -> dict: #only fetch onece in lifetime then save to json
        ...
        try:               
            ...
            with self._comm_lock: #wait for result                
                if rand_uuid != self._ws_cmds.id:
                    self._logger.error(f"fetch_exchange_info() error: uuid mismatch send= {rand_uuid} response= {self._ws_cmds.id}")  
                self._ws_cmds.id = ""   
            ...
            self._logger.error(f"fetch_exchange_info() error: {e}")
            return self._exchange_info_data
    ...
    def fetch_kline(self, symbol1, symbol2, interval, num_data:int = 100) -> list[dict]:    
        ...
        try:
            ...
            with self._comm_lock: #wait for result                       
                if rand_uuid != self._ws_cmds.id:
                    self._logger.error(f"fetch_kline() error: uuid mismatch send= {rand_uuid} response= {self._ws_cmds.id}")  
                self._ws_cmds.id = ""   
    ...
    def send_trade(self, open_trade: Trade) -> Trade:
        ...
        try: 
            ...
            with self._comm_lock:                    
                if rand_uuid != self._ws_cmds.id:
                    self._logger.error(f"send_trade() error: uuid mismatch send= {rand_uuid} response= {self._ws_cmds.id}")  
                self._ws_cmds.id = ""   
    ...
    # ---------------------------------------------------------
    @staticmethod
    def _format_ping_response(response_result : PingResponse) ->str:  
        text = ""
        if isinstance(response_result,PingResponse):
            if "error" in response_result:
                ...
                text = f"id= {response_result.id}, Rate Limits: Interval= {rate_limits.interval} num= {rate_limits.interval_num}, Limits= {rate_limits.count} / {rate_limits.limit}"
        return text 
