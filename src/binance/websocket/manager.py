...
class WebsocketManager:
    ...
    def _format_order_place_msg(self, trade:Trade):
        ...
        if float(trade.quantity1) < 0.0:
            side="SELL"
            quote_order_qty= 0
            quantity = self.round_down(abs(float(trade.quantity1)), round_order) #When selling I want to sell the amount of Symbol1 I will recive Symbol2 depending on the market                        
        else:
            side="BUY"
            quote_order_qty= self.round_down(abs(float(trade.quantity2)), round_order)#When buying spend amount of symbol2 and recive amount of symbol1 depending on market
            quantity= 0        
        websocet_msg = {
                    "side" : side,
                    "symbol" : pair,
    ...
    def _format_ping_response(response_result : PingResponse, time_now) ->str:  
        ...
        if isinstance(response_result,PingResponse):
            if "error" in response_result:
                ...
                rate_limits = response_result.rate_limits[0]
