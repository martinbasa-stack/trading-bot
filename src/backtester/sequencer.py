from src.strategy.trades import TradeAnalyzer, Trade
from src.strategy.indicators import IndicatorCompute
from src.strategy.assets import AssetManagerResult, AssetAnalyzer
from src.strategy.record_HL.manager import HLRecordManager 

from src.binance import StreamManager 
from src.market_history import MarketHistoryManager, PairHistory,IntervalData
...
class Sequencer:    
    ...
    def __init__(self, 
        ...
        self._strategy_id :str = "backtester"
    ...
    def run(self) -> bool:
        ...
        try:
            with self._lock:
                ...
                if self.strategy.candle_close_only:
                    self._active_step += 1
    ...
    def get_now_sim(self) -> int:
        ...
        try:  
            with self._lock:
                if not self._data_active[self.strategy.candle_interval]:
                    return None
                time_open = self._data_active[self.strategy.candle_interval].time_open[-1]
    ...
    def get_fng_sim(self):
        try:
            interval_s = self.strategy.candle_interval
    ...
    def get_available(self, symbol: str) -> float:
        ...
        try:            
            with self._lock:
                if symbol == self.strategy.symbol1:
    ...
    def _load_all(self):
        with self._lock:
            self.strategy = self._get_by_id(self._strategy_id)
            s = self.strategy
    ...
    def _load_active_table(self):
        s_interval = self.strategy.candle_interval
    ...
    def _update_saved(self, interval):
        if not self.strategy.candle_close_only:
