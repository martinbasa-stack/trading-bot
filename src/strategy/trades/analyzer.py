from .models import  PnL, AverageSum
...
class TradeAnalyzer:
    ...
    def get_all_avgs_list(self, strategy_id:int) -> Tuple[list[AverageSum], list[AverageSum], list[AverageSum]]:
        ...
        try:
            ...
            self._logger.error(f"get_all_avgs_list() error: {e}")
    ...
    def get_min_max_price(self, strategy_id, record: HLRecord) -> Tuple[float, float]:
        ...
        try:
            with self._lock:            
                ...
                if self._strategy.last_trade_as_min_max and last_trade_price:
                    return last_trade_price, last_trade_price        
                
                min_price, max_price = self._min_max_price(last_trade_price,lookback, record)
                
                return min_price, max_price
        except Exception as e:
            self._logger.error(f"get_min_max_price() ID: {strategy_id} error: {e}")
    ...
    def get_kline_id_of_last_trade(self, strategy_id) -> int:
        ...
        try:
            with self._lock:
                ...
                if not hist_table:
                    raise RuntimeError(f"No history table for ID: {self._strategy.idx} pair: {self._pair} type: {self._strategy.type_s}")
                
                arr = hist_table.time_close
                trade_time = trade_table[-1].timestamp

                np_idx = np.searchsorted(arr, trade_time)
                np_idx = len(arr) - np_idx - 1
            ...
            self._logger.error(f"get_kline_id_of_last_trade() error: {e}")
    ...
    def get_trade_enable(self, strategy_id) -> bool:
        ...
        try:
            with self._lock:
                if strategy_id == "backtester": #Not nice that is in this class try to change
                    return True
                hist_table:IntervalData = self._hist_table()
                if not hist_table:
                    raise RuntimeError(f"No history table for ID: {self._strategy.idx} pair: {self._pair} type: {self._strategy.type_s}")
            ...
            self._logger.error(f"get_trade_enable() error: {e}")        
    ...
    def _min_max_price(self, last_trade_price, lookback, record: HLRecord) -> Tuple[float, float]: 
        hist_table: IntervalData = self._hist_table()
        if not hist_table:
            raise RuntimeError(f"No history table for ID: {self._strategy.idx} pair: {self._pair}")
        ...
        if lookback < 1: #Trade in the last candle #Save to dictionary maximum FOR BUY
            ...
            min_price = min(last_trade_price, record.low) if last_trade_price != 0 else record.low # Check if the price went below
            ...
            #Write last trade price to the TA anlasys arry to use it insted of actual value
            low[len(low)-1 -lookback] = min(record.low, last_trade_price) if last_trade_price != 0 else record.low #Write last trade price to the TA anlasys arry to use it insted of actual value
    ...
    def _averadge_cost_pnl(self, strategy_id:int, last_close) -> Tuple[list[PnL],  list[AverageSum]]:
        ...
        for trade in trade_table:        
            ...
            max_s1 = max(max_s1, abs(sum1))
            max_s2 = max(max_s2, abs(sum2))
            ...
            if position == 0:
                ...
                pnl_list.append(self._calc_pnl(cost_basis,position,last_close,realized_pnl, max_s2))
                continue
    ...
    @staticmethod
    def _calc_pnl(cost_basis,position,last_close, realized_pnl, max_s2) -> PnL:
            avg_cost = cost_basis / abs(position) if position != 0 else 0.0
            ...
            unreal_percent=unrealized_pnl / abs(max_s2) * 100 if max_s2 != 0 else 0
            real_percent=realized_pnl / abs(max_s2) * 100 if max_s2 != 0 else 0
            total_percent=total_pnl / abs(max_s2) * 100 if max_s2 != 0 else 0
