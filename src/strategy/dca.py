...
from src.market_history import MarketHistoryManager
...
class DCAstrategy:    
    ...
    def __init__(self, 
                 ...
                 stream_manager_obj: StreamManager,
        ...
        self._stream_manager_obj: StreamManager = stream_manager_obj
        self._record_obj : HLRecordManager = record_obj
        self._market_history_obj : MarketHistoryManager = market_history_obj
    ...
    def get_trigger_compute(self, strategy_id) -> TriggerComputeResult:  
        ...
        with self._lock:      
            if not self._compute(strategy_id):
                return None
            return copy.deepcopy(self._compute_result)

    def _compute(self, idx) -> Trade:
        try:
            ...
            #Update price data 
            self._new_close = self._stream_manager_obj.get_close(pair) #get close price
            ...
            percent_change_dip = (max_price - self._new_close) / max_price * 100
            dca_buy_trigger = percent_change_dip > self._strategy.asset_manager.dip_buy
            #Check changes form Min newSellPumpOk
            percent_change_pump = (self._new_close - min_price) / min_price * 100
    ...
    def _format_trade(self) -> Trade:  
        try:   
            ...
            if trigger_buy or trigger_sell:
                ...
                if not self._strategy.asset_manager.paper_t:                    
