...
class DCAstrategy:    
    ...
    # Public 
    # ==================================================================  
    # Return trad if conditions are ok
    # ---------------------------------------------------------
    ...
    def _compute(self, idx):
        try:
            ...
            # Sum triggerts
            sum_buy_trigger = trade_enable and buy_ind_trade_en and dca_buy_trigger and self._asset_result.s2_balance_ok
            sum_sell_trigger = trade_enable and sell_ind_trade_en and dca_sell_trigger and self._asset_result.s1_balance_ok
            # if both are triggered select only the one that is not to be saved
            if sum_buy_trigger and sum_sell_trigger:
                if self._strategy.asset_manager.target != "None":
                    if self._strategy.asset_manager.symbol_index == 1:
                        sum_sell_trigger = False
                        dca_sell_trigger = False
                    else:
                        sum_buy_trigger = False
            ...
            self._compute_result= TriggerComputeResult(
                ...
                buy_sum_trigger = sum_buy_trigger,
                ...
                sell_sum_trigger  = sum_sell_trigger,
            ...
            self._logger.error(f"DCA compute ID:{idx} pair: {pair} error: {e}")
            
    # Format trade after compute is done!
    # ---------------------------------------------------------
    def _format_trade(self) -> Trade:  
        try:   
            ...
            if (trigger_buy):
                ...
                s2_qt = -self.round_down(self._asset_result.to_buy, self._strategy.round_order)
                percent_change = self._compute_result.percent_change_dip
                side = "BUY"
            #Sell conditions
            elif (trigger_sell):            
                s1_qt = -self._asset_result.to_sell/self._new_close
                s2_qt = self.round_down(self._asset_result.to_sell, self._strategy.round_order)
