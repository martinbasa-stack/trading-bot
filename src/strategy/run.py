...
from src.tray.commands import notify_put
...
async def _trade_on_raydium(s: StrategyConfig):
    ...
    if not open_trade:        
        ...
        if close_trade:
            if close_trade.idx == "Error":
                open_trade = trade_manager_obj.get_send(s.idx)

    if open_trade:
        send_trade = solana_man_obj.send_trade(s.idx, open_trade, 0.5)
        if send_trade:
            await asyncio.sleep(1)
            msg = _format_telegram_msg(send_trade, "Raydium")
    ...
    if close_trade:
        if close_trade.idx != "Error":
            ...
            update_assets_q("Solana")
            msg = _format_telegram_msg(close_trade, "Raydium")
            telegram_obj.send_msg(msg)
            notify_put(msg)
...
async def _trade_on_binance(s:StrategyConfig):
    ...
    if open_trade:
        ...
        msg = _format_telegram_msg(open_trade, "Binance" , round_order)
        telegram_obj.send_msg(msg)
        notify_put(msg)
        if close_trade:
            trade_manager_obj.set_close(s.idx, close_trade)
            msg =  _format_telegram_msg(close_trade, "Binance" , round_order)        
            telegram_obj.send_msg(msg)
            notify_put(msg)
...
def _format_telegram_msg(trade: Trade, exchange : str, round_order = 5):   
    try: 
        ...
        if trade.commision > 0:
            ...
            title = (f"Order FILLED on {exchange}:\n "
                        f"id : {trade.idx} \n "
                    )
        else:
            append = ""
