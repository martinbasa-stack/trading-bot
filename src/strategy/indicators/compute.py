...
class IndicatorCompute:
    ...
    def __init__(self, 
        ...
        Args:
            ...
            fear_gread(float):
                value of Fear and Gread
        """
        self._get_hist_table: function = get_hist_table
        self._strategies_obj : StrategyManager = strategies_obj
        self._get_last_close : function = get_close
        self._all_avg : function = get_all_avg
        self._pair:str = ""
        self._asset_manager = None
        self._last_close: float = 0.0
        self._fng = fear_gread
    ...
    def get_sell_list(self, strategy_id:int) -> list[IndicatorResult]:
        ...
        try:
            ...
            self._logger.error(f"get_sell_list() error: {e}")
    ...
    def _populate_instance_vars(self, strategy_id):
        ...
        self._avg_entry, self._avg_cost, self._avg_exit = self._all_avg(strategy_id)
    ...
    def _generate_result_list(self, indicators_config : list[IndicatorConfig]) -> list[IndicatorResult]:
        ...
        for ind_config in indicators_config:
            match ind_config.type_i:
                ...
                case "AvrageEntry":
                    result = self._average_indicator(self._avg_cost.avg, "Average Entry", ind_config)
                case "AvrageExit":
                    result = self._average_indicator(self._avg_cost.avg, "Average Exit", ind_config)
                case "Price":
    ...
    def _adx_indicator(self, config:IndicatorConfig) -> IndicatorResult:
        ...
        result = self._ind_compute(100, config.trigger, ind_out, config)
        result.dis_text = f"{config.type_i} {config.value1} {config.interval} | Trigger"
        return result
    ...
    def _bb_indicator(self, config:IndicatorConfig) -> IndicatorResult:
        ...
        result.dis_text = f"BB {config.output_select} {config.value1} {config.interval} | Price"
        return result
    ...
    def _trigger_select(self, value, config:IndicatorConfig) -> Tuple[float, str]:
        ...
        dis_text = f"{config.type_i} {config.value1} {config.interval} | {sel} "
        match sel:
            case "SMA":
                ta_list = ta.SMA(value, config.trigger)  
                trigg_val = ta_list[-1]
                dis_text +=f"{config.trigger} {config.interval}"
            case "EMA":
                ta_list = ta.EMA(value, config.trigger)
                trigg_val = ta_list[-1]
                dis_text +=f"{config.trigger} {config.interval}"
            case _: #For price
    ...
    def _delta(divider, val_to_compre, trigger) -> float:
        if divider != 0: #check that trigger is positive if not price change revers calculation 
            return (val_to_compre - trigger)/divider #Calc price change from trigger -Down +Up 
        else: 
            return 1 #set to 1 this is when no data for avradges
