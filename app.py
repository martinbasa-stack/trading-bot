...
from src import  settings_obj, strategy_run
from src.strategy import asset_manager_obj
from src.telegram import send_telegram_msg
from src.binance import stream_manager_obj, stream_main,  ws_manager_obj, websocet_main #ping_websocet, fetch_exchange_info, fetch_user_data
...
def _main():
    ...
    while True:            
        ...
        run_loop = not first_run and (not stream_manager_obj.all_streams_available() or not stream_manager_obj.all_data_current(data_current_time))
        while run_loop:
        ...
        if consol_update_count > 3600 * 1.5 or first_run:
            consol_update_count = 0
            print(f"Current active streams: {stream_manager_obj.get_active_list()}")
...
def _initialize():
    ...
    try:
        ...
        send_telegram_msg("Bot startup finished")
